...
func HttpServe() error {
	...
	ra.HandleFunc("/Clients/{ClientSid:[0-9a-fA-F]{32}}{format:(?:\\.xml|\\.csv|\\.json)?}", GetApplicationClient).Methods("GET")
	ra.HandleFunc("/Clients/{ClientSid:[0-9a-fA-F]{32}}{format:(?:\\.xml|\\.csv|\\.json)?}", DeleteApplicationClient).Methods("DELETE")
	ra.HandleFunc("/Clients/{ClientSid:[0-9a-fA-F]{32}}{format:(?:\\.xml|\\.csv|\\.json)?}", CreateApplicationClient).Methods("POST")
